...
bool PingClass::ping(IPAddress dest, byte count) { return PingClass::pingMs(dest, count, 1000, 32, 1000); }

bool PingClass::ping(const char *host, byte count) { return PingClass::pingMs(host, count, 1000, 32, 1000); }
...
bool PingClass::pingMs(IPAddress dest, byte count, int timeout_ms, int size, int interval_ms) {
    ...
    _options.interval_ms = interval_ms;
...
bool PingClass::pingMs(const char *host, byte count, int timeoutMs, int size, int intervalMs) {
...
float PingClass::averageTime() { return _avg_time; }
...
void PingClass::_ping_recv_cb(void *opt, void *resp) {
    ...
    DEBUG_PING("DEBUG: ping reply\n"
               ...
               "\ttotal_time = %f s\n"
               "\tping_err = %d \n",
               ping_resp->total_count, ping_resp->resp_time, ping_resp->seqno, ping_resp->timeout_count, ping_resp->bytes, ping_resp->total_bytes, ping_resp->total_time,
               ping_resp->ping_err);
    ...
    if (_success + _errors != _expected_count) {
        DEBUG_PING("Something went wrong: _success=%d and _errors=%d do not "
                   "sum up to _expected_count=%d\n",
